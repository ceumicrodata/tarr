...
from tarr.dag_config import NEXT, STOP, Node

class Test_PyDagConfigReader(unittest.TestCase):
    ...
    def test_implicit_next(self):
        ...
        dag = m.PyDagConfigReader().from_config(config)

        node1 = dag.node_by_name('node1')
        self.assertEqual(('node2', 'node2'), (node1.nn_success, node1.nn_fail))

    def test_implicit_next_on_last_node_is_STOP(self):
        config = [
            Node('node1', 'impl1'),
            Node('node2', 'impl2')
        ]
        ...
        node2 = dag.node_by_name('node2')
        self.assertEqual((STOP, STOP), (node2.nn_success, node2.nn_fail))

    def test_STOP_on_success(self):
        config = [
            Node('node1', 'impl1', on_success=STOP),
            Node('node2', 'impl2')
        ]
        dag = m.PyDagConfigReader().from_config(config)

        node1 = dag.node_by_name('node1')
        self.assertEqual((STOP, 'node2'), (node1.nn_success, node1.nn_fail))

    def test_STOP_on_fail(self):
        config = [
            Node('node1', 'impl1', on_fail=STOP),
            Node('node2', 'impl2')
        ]
        dag = m.PyDagConfigReader().from_config(config)

        node1 = dag.node_by_name('node1')
        self.assertEqual(('node2', STOP), (node1.nn_success, node1.nn_fail))

    def test_impl_is_set(self):
        config = [
            Node('node1', 'impl1'),
            Node('node2', 'impl2')
        ]
        ...
        node1 = dag.node_by_name('node1')
        node2 = dag.node_by_name('node2')
        self.assertEqual(('impl1', 'impl2'), (node1.impl, node2.impl))
